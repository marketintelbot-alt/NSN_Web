...
export function createBusinessNotificationEmail(booking: AdminBooking) {
  ...
  const details = [
    ...
    ['Booking Source', booking.createdBy === 'admin' ? 'Admin dashboard' : 'Website booking page'],
...
export function createCustomerConfirmationEmail(booking: AdminBooking) {
  ...
  const html = renderEmailShell(
    ...
    `North Shore Nautical has reserved your slot for <strong style="color:#06131f;">${escapeHtml(formattedLaunchTime)}</strong>.`,
    `
      <div style="margin-top:28px;padding:22px 24px;border-radius:20px;background:#f4f8fa;border:1px solid rgba(6,19,31,0.08);">
        ...
        <p style="margin:12px 0 0;color:#516474;font-size:14px;line-height:1.8;">
          ...
          Reserved For: <strong style="color:#06131f;">${escapeHtml(booking.fullName)}</strong>
        </p>
      </div>
      <p style="margin:24px 0 0;color:#516474;font-size:14px;line-height:1.8;">
        If any details change, simply reply to this email and North Shore Nautical will update the booking for you. If you do not see this message in your inbox later, your reservation is still on file.
      </p>
    `,
  )
