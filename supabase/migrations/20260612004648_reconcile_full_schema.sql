...
alter table public.service_requests
  drop constraint if exists service_requests_state_consistency_check;

alter table public.service_requests
  add constraint service_requests_state_consistency_check
  check (
    (booking_status <> 'draft' or payment_status = 'not_started')
    and (booking_status <> 'confirmed' or payment_status = 'captured')
    and (booking_status <> 'completed' or payment_status = 'captured')
    and (booking_status <> 'refunded' or payment_status = 'refunded')
    and (payment_status <> 'refunded' or booking_status = 'refunded')
    and (booking_status <> 'declined' or payment_status not in ('captured', 'refunded'))
    and (payment_status <> 'authorized' or booking_status = 'pending_review')
  );
...
-- All data access is brokered by the backend service-role client.
